...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: locale === "en" ? `Privacy | ${SITE_NAME}` : `Zasebnost | ${SITE_NAME}`, alternates: { canonical: privacyPath(locale) }, robots: { index: true, follow: true } };
}
...
export default async function PrivacyPage() {
  ...
  return <main className={styles.page}>
    <header className={styles.header}><div className={styles.headerInner}><Link className={styles.brand} href={home}>{brandMarkSrc ? <img className={styles.brandMark} src={brandMarkSrc} alt="" width={30} height={30} /> : null}<span className={styles.brandWord}>Guest<span> Mosaic</span></span></Link><Link className={styles.back} href={home}>{en ? "Back to gallery" : "Nazaj na galerijo"}</Link></div></header>
    <article className={styles.article}>
      {en ? <>
      <p className={styles.eyebrow}>Legal document</p><h1>Privacy Policy</h1><p className={styles.updated}>Version: 31 July 2026</p>
      <p>This policy explains how data is processed in Guest Mosaic. For questions or to exercise your rights, email <a href="mailto:info@eventaj.si">info@eventaj.si</a>.</p>
      ...
      <h2>3. Processors</h2><p>We use Cloudflare for hosting, database, image storage and video processing. Stripe processes payments; Guest Mosaic does not store full card details. Specific optional features may use documented external providers under contractual and security restrictions.</p>
      ...
      <p>Ta politika pojasnjuje obdelavo podatkov v Guest Mosaic. Za vprašanja ali uveljavljanje pravic pišite na <a href="mailto:info@eventaj.si">info@eventaj.si</a>.</p>
      ...
      <h2>3. Ponudniki obdelave</h2><p>Za gostovanje, zbirko podatkov, shrambo slik in obdelavo videov uporabljamo Cloudflare. Plačila obdeluje Stripe; Guest Mosaic ne hrani celotnih podatkov plačilne kartice. Posamezne dodatne funkcije lahko uporabljajo dokumentirane zunanje ponudnike pod pogodbenimi in varnostnimi omejitvami.</p>
