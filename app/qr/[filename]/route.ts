...
import { appUrlForLocale, localeFromRequest } from "@/lib/i18n/locale";
...
export async function GET(request: Request, { params }: { params: Promise<{ filename: string }> }) {
  ...
  if (!match || !publicAccessPointCodeSchema.safeParse(match[1]).success) {
    return problem(404, "QR_NOT_FOUND", notFoundMessage);
  }

  const [, publicCode, format] = match;
  const point = await findActiveAccessPoint(publicCode);
  if (!point) return problem(404, "QR_NOT_FOUND", notFoundMessage);

  const target = accessPointTarget(appUrlForLocale(env, locale), publicCode);
  const options = {
