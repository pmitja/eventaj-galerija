...
import { localizedMarketingPath, privacyPath, termsPath } from "@/lib/i18n/routes";

export async function generateMetadata(): Promise<Metadata> {
  const locale = await getRequestLocale();
  return { title: locale === "en" ? `Terms of Use | ${SITE_NAME}` : `Pogoji uporabe | ${SITE_NAME}`, alternates: { canonical: termsPath(locale) }, robots: { index: true, follow: true } };
}
...
export default async function TermsPage() {
  ...
  return <main className={styles.page}>
    ...
    <article className={styles.article}>
      ...
      <p>These terms govern the use of Guest Mosaic by event organisers and guests who upload photos and videos through a link or QR code.</p>
      ...
      <h2>7. Contact</h2><p>For questions, content reports or deletion requests, email <a href="mailto:info@eventaj.si">info@eventaj.si</a>. Details about personal data processing are in the <Link href={privacyPath(locale)}>Privacy Policy</Link>.</p>
      </> : <>
      <p className={styles.eyebrow}>Pravni dokument</p><h1>Pogoji uporabe</h1><p className={styles.updated}>Različica: 31. julij 2026</p>
      <p>Ti pogoji urejajo uporabo Guest Mosaic za organizatorje dogodkov in goste, ki prek povezave ali QR kode nalagajo fotografije in videe.</p>
      ...
      <h2>7. Kontakt</h2><p>Za vprašanja, prijavo vsebine ali zahtevek za izbris pišite na <a href="mailto:info@eventaj.si">info@eventaj.si</a>. Podrobnosti o obdelavi osebnih podatkov so v <Link href={privacyPath(locale)}>Politiki zasebnosti</Link>.</p>
      </>}
    </article>
  </main>;
}
