...
export default async function CheckoutSuccessPage({ searchParams }: { searchParams: Promise<{ session_id?: string }> }) {
  ...
  return <main className={checkoutPageClass}><div className={checkoutShellClass}>
    <header className={checkoutHeadingClass}>
      ...
      <span className={checkoutHeadingTextClass}>{ready ? (en ? "Add your event details now. We also sent this private setup link by email." : "Zdaj dodaj podatke dogodka. Zasebno povezavo za nastavitev smo poslali tudi po e-pošti.") : (en ? "Stripe is still confirming the payment. Refresh this page in a few moments." : "Stripe še potrjuje plačilo. Stran čez nekaj trenutkov osveži.")}</span>
    </header>
    {ready ? <Card className="mx-auto mt-8 max-w-[650px] text-center shadow-[0_18px_48px_rgba(79,18,47,.09)]"><CardContent>
      ...
      {galleryUrl ? <div className="mb-5 grid gap-2.5">
        <a className={cn(successLink, "bg-brand text-white! shadow-[0_1px_2px_rgba(225,29,72,.3)]")} href={galleryUrl}><QrCode className="size-[18px] flex-none" aria-hidden="true" />{en ? "Set up your event" : "Nastavi svoj dogodek"}</a>
