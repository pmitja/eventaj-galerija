...
import { listLiveMediaComments, listSlideMediaComments } from "@/lib/repositories/media-comments";
...
export async function GET(_request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  return Response.json({
    ...
    media: media.map((item) => ({
      ...
      comments: slideComments[item.public_id] ?? [],
    })),
    engagement,
    comments,
